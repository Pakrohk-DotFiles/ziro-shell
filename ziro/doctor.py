...
def _check_starship_toml() -> Check:
    ...
    if conf.is_file():
        contents = conf.read_text(encoding="utf-8", errors="replace")
        if "Generated by .prompt.local" in contents:
            return Check("starship.toml", True, "generated (managed by Ziro)")
